...
- Enforced view doit être VIDE pour `mcp-gateway` et `furyssh`:
  ssh -o BatchMode=yes -o ConnectTimeout=5 supabase 'docker exec -i supabase-db psql -U postgres -d postgres -X -q -P pager=off -c "select hostname, container_name, lifecycle_state, expected_state, severity from public.v_expected_docker_services_enforced where hostname in (''mcp-gateway'',''furyssh'') order by hostname, container_name;"' | sed -n '1,220p'
