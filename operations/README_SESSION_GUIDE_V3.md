# README — Démarrage de session (canonique) (V3)
...
## 3) Commandes après sauvegarde (repointer + commit + vérifs bornées)

```bash
cd /home/furycom/manual-docs
set -euo pipefail

echo "=== repoint README_SESSION_GUIDE_LATEST -> V3 ==="
ln -sfn README_SESSION_GUIDE_V3.md operations/README_SESSION_GUIDE_LATEST.md

echo
echo "=== verify pointer + header (bounded) ==="
ls -la operations/README_SESSION_GUIDE_LATEST.md
readlink -f operations/README_SESSION_GUIDE_LATEST.md || true
sed -n '1,40p' operations/README_SESSION_GUIDE_LATEST.md

echo
echo "=== git add/commit ==="
git add operations/README_SESSION_GUIDE_V3.md operations/README_SESSION_GUIDE_LATEST.md
git commit -m "docs: README_SESSION_GUIDE V3 (RAG payload q)"
